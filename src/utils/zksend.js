...
export async function listCreatedLinks(
  { address, cursor, network, contract = MAINNET_CONTRACT_IDS, ...linkOptions },
  fetchFn // HERE IS THE KEY PARAMETER
) {
  const gqlClient = new SuiGraphQLClient({
    ...
    fetch: fetchFn, // HERE IS THE INFERENCE
  });
  ...
  const links = (
    await Promise.all(
      transactionBlocks.nodes.map(async (node) => {
        ...
        const address = bcs.Address.parse(fromB64(input.bytes));
